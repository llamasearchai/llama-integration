...
def mock_gateway(mock_config_manager, mock_provider, mock_router):
    ...
    with patch(
        "llama_integrations.gateway.ConfigManager", return_value=mock_config_manager
    ), patch("llama_integrations.gateway.APIKeyManager"), patch(
        "llama_integrations.gateway.AuditLogger"
    ), patch(
...
@pytest.mark.asyncio
async def test_completion_with_routing_strategy(
    mock_gateway, mock_provider, mock_router
):
