...
try:
    ...
    raise ImportError(
        "MLX is required for the router. " "Install it with 'pip install mlx'."
    )
        ...
        for _ in range(num_layers):
            encoder_layers.append(
                nn.TransformerEncoderLayer(
                    embedding_dim, num_heads, hidden_dim, dropout=0.1
                )
            )
...
class MLXRouter:
    ...
    def __init__(
        ...
        if (
            ...
            logger.warning(
                "No model path provided or model not found. "
                "Using simulated router model."
            )
        ...
        if self.is_simulated:
            ...
            if "code" in text.lower() or "python" in text.lower():
                ...
                return random.choices(
                    list(self.provider_mapping.values()), weights=weights, k=1
                )[0]
        else:
